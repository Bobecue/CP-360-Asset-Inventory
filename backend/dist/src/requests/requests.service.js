...
let RequestsService = class RequestsService {
    ...
    async onModuleInit() {
        ...
        const defaultPasswordHash = '$2b$10$q7Jq5HiZdRkZ4lSqvtR4z.JOwjMZME.d8Q8ifcOYywevdb1.PkXn6';
    ...
    mapRequestToDto(r) {
        ...
        try {
            if (r.purpose && r.purpose.startsWith('{')) {
                ...
                if (parsed.returnedAt)
                    returnedAtStr = parsed.returnedAt;
                if (parsed.siteId)
        ...
        let statusDto = 'PENDING';
        if (r.status === 'PENDING_OPS_APPROVAL')
            statusDto = 'PENDING_OPS_APPROVAL';
        if (r.status === 'APPROVED')
            statusDto = 'APPROVED';
        if (r.status === 'READY_FOR_PICKUP')
            statusDto = 'READY_FOR_PICKUP';
        if (r.status === 'PENDING_PROCUREMENT')
            statusDto = 'PENDING_PROCUREMENT';
        ...
        if (r.status === 'AWAITING_CONFIRMATION')
            statusDto = 'AWAITING_CONFIRMATION';
        if (r.status === 'ITEM_RECEIVED')
            statusDto = 'ITEM_RECEIVED';
    ...
    async create(dto, userEmail) {
        ...
        if (!item) {
            ...
            const prefix = catName.toLowerCase().includes('laptop') ? 'LAP' :
        ...
        if (!requester) {
            throw new common_1.NotFoundException('No requester user found in database.');
        }
        if (!dto.siteId) {
        ...
        if (!siteObj) {
            throw new common_1.NotFoundException(`Site with ID/Name ${dto.siteId} not found.`);
        }
        ...
        const r = await this.prisma.request.create({
            data: {
                id: `req-${(0, uuid_1.v4)().substring(0, 8)}`,
    ...
    async getSummary(user) {
        ...
        const approvedThisMonth = items.filter(r => approvedStatuses.includes(r.status) && new Date(r.createdAt) >= startOfMonth);
        const rejectedThisMonth = items.filter(r => r.status === 'REJECTED' && new Date(r.createdAt) >= startOfMonth);
        ...
        pending.forEach(r => {
            const u = (r.urgency || 'NORMAL').toUpperCase();
    ...
    async findMine(userEmail, status) {
    ...
    async approve(id, comment, approverEmail) {
        throw new common_1.BadRequestException('Direct approval is disabled. Please use approveStaff or approveOps.');
    }
    async reject(id, comment, approverEmail) {
        let approverConnect = undefined;
        if (approverEmail) {
            const u = await this.prisma.user.findUnique({ where: { email: approverEmail } });
            if (u) {
                if (u.role === 'SUPER_ADMIN') {
                    throw new common_1.BadRequestException('Super Admins are unauthorized to reject requests.');
                }
        ...
        if (currentReq.status !== 'PENDING_APPROVAL' && currentReq.status !== 'PENDING_OPS_APPROVAL') {
            throw new common_1.BadRequestException(`Cannot reject a request that is currently ${currentReq.status}.`);
        }
    ...
    async approveStaff(id, staffEmail, comment) {
        const u = await this.prisma.user.findUnique({ where: { email: staffEmail } });
        if (!u) {
            throw new common_1.NotFoundException('User not found');
        }
        if (u.role !== 'INVENTORY_STAFF') {
            throw new common_1.ForbiddenException('Only Inventory Staff can perform staff approval.');
        }
        const currentReq = await this.prisma.request.findUnique({ where: { id } });
        if (!currentReq) {
            throw new common_1.NotFoundException('Request not found');
        ...
        if (currentReq.status !== 'PENDING_APPROVAL') {
            throw new common_1.BadRequestException(`Cannot staff-approve a request that is currently ${currentReq.status}.`);
        }
        const r = await this.prisma.request.update({
            where: { id },
            data: {
    ...
    async approveOps(id, opsEmail, comment) {
        const u = await this.prisma.user.findUnique({ where: { email: opsEmail } });
        if (!u) {
            throw new common_1.NotFoundException('User not found');
        }
        if (u.role !== 'ADMIN') {
            throw new common_1.ForbiddenException('Only Ops Manager can perform ops approval.');
        }
        ...
        if (currentReq.status !== 'PENDING_OPS_APPROVAL') {
            throw new common_1.BadRequestException(`Cannot ops-approve a request that is currently ${currentReq.status}.`);
        }
        if (currentReq.staffApprovedById === u.id) {
            throw new common_1.ForbiddenException('Separation of duties: The same user cannot perform both staff and ops approvals.');
        }
        let parsedPurpose = {};
        try {
    ...
    async preparePickup(id, staffEmail, comment) {
        const u = await this.prisma.user.findUnique({ where: { email: staffEmail } });
        if (!u) {
            throw new common_1.NotFoundException('User not found');
        }
        if (u.role !== 'INVENTORY_STAFF') {
            throw new common_1.ForbiddenException('Only Inventory Staff can prepare/stage items for pickup.');
        }
        const currentReq = await this.prisma.request.findUnique({ where: { id } });
        if (!currentReq) {
            throw new common_1.NotFoundException('Request not found');
        ...
        if (currentReq.status !== 'APPROVED') {
            throw new common_1.BadRequestException(`Cannot mark a request as Ready for Pickup if it is currently ${currentReq.status}.`);
        }
        const r = await this.prisma.request.update({
            where: { id },
            data: {
    ...
    async release(id, assetId, releaserEmail) {
        const u = await this.prisma.user.findUnique({ where: { email: releaserEmail } });
        if (u && u.role === 'SUPER_ADMIN') {
            throw new common_1.BadRequestException('Super Admins are unauthorized to release requests.');
        }
        const req = await this.prisma.request.findUnique({ where: { id }, include: { item: true, requester: true } });
        if (!req)
            throw new common_1.NotFoundException('Request not found');
        if (req.status !== 'READY_FOR_PICKUP') {
            throw new common_1.BadRequestException('Cannot release a request that is not ready for pickup.');
        }
        if (u && u.id === req.requesterId) {
            throw new common_1.BadRequestException('A user cannot release their own request.');
        }
        return await this.prisma.$transaction(async (tx) => {
            let asset = await tx.asset.findUnique({ where: { tagCode: assetId } });
            if (asset && asset.status !== 'AVAILABLE') {
                throw new common_1.BadRequestException(`Asset Tag '${assetId}' is already checked out or unavailable. Please use a different tag.`);
            }
            if (!asset) {
    ...
    async cancel(id) {
        ...
        if (currentReq.status !== 'PENDING_APPROVAL' && currentReq.status !== 'APPROVED' && currentReq.status !== 'READY_FOR_PICKUP') {
            throw new common_1.BadRequestException(`Cannot cancel a request that is currently ${currentReq.status}.`);
        }
        const r = await this.prisma.request.update({
            where: { id },
            data: {
    ...
    async return(id, returnComment, returnerEmail) {
        const r = await this.prisma.request.findUnique({
            where: { id }
        });
        if (!r)
        ...
        if (r.status !== 'RELEASED' && r.status !== 'AWAITING_CONFIRMATION' && r.status !== 'ITEM_RECEIVED') {
            throw new common_1.BadRequestException(`Cannot return a request that is currently ${r.status}.`);
        }
        let adminUser = null;
        if (returnerEmail) {
            adminUser = await this.prisma.user.findUnique({ where: { email: returnerEmail } });
            if (adminUser && adminUser.role === 'SUPER_ADMIN') {
                throw new common_1.BadRequestException('Super Admins are unauthorized to process returns.');
            }
        }
        let parsedPurpose = {};
        try {
    ...
    async addComment(id, comment, userEmail) {
        const r = await this.prisma.request.findUnique({
            where: { id }
        });
        if (!r)
    ...
    async confirmReceipt(id, userEmail) {
        const user = await this.prisma.user.findFirst({
            where: { email: userEmail }
        });
        if (!user) {
        ...
        if (req.status !== 'AWAITING_CONFIRMATION') {
            throw new common_1.BadRequestException(`Cannot confirm receipt for a request in status ${req.status}.`);
        }
        if (req.requesterId !== user.id) {
            throw new common_1.ForbiddenException('Only the original requester can confirm receipt of this item.');
        }
        const updated = await this.prisma.request.update({
            where: { id },
            data: {
