...
with open('RequestTimeline.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# We want to replace everything from `const mapStatus` down to the `return (` statement with a new implementation.
start_str = "  const mapStatus = (statusStr: string): string => {"
...
new_logic = """  const Badge = ({ children, type }: { children: React.ReactNode, type: 'ops' | 'inv' }) => {
  ...
  const buildNodes = () => {
    ...
    let hasStaffApproved = false;
    let hasOpsApproved = false;
...
# Next, we need to replace the rendering loop.
render_start_str = "          {displayHistory.map((evt, idx) => {"
render_end_str = "          })}\n        </div>"

render_start_idx = content.find(render_start_str)
render_end_idx = content.find(render_end_str)

if render_start_idx == -1 or render_end_idx == -1:
    print("Could not find render loop block")
    exit(1)

new_render_logic = """          {displayNodes.map((node, idx) => {
            return (
...
content = content[:render_start_idx] + new_render_logic + content[render_end_idx:]

with open('RequestTimeline.tsx', 'w', encoding='utf-8') as f:
    f.write(content)

print("Updated RequestTimeline.tsx successfully!")
