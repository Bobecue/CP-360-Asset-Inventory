      ...
      {selectedReqIds.length > 0 && (
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
            ...
            {canApprove && selectedRequests.some(r => ['APPROVED', 'PENDING_PROCUREMENT'].includes(r.status as string)) && !selectedRequests.some(r => ['PENDING', 'PENDING_APPROVAL', 'PENDING_OPS_APPROVAL'].includes(r.status as string)) && (
              <button
            ...
            {canRelease && selectedRequests.some(r => ['READY_FOR_PICKUP'].includes(r.status as string)) && (
              <button
                          ...
                          <button
                            onClick={() => {
                              setConfirmState({
                                ...
                                onConfirm: async (val) => {
                                  if (!val || !val.trim()) return;
                                  await onReview(req.id, 'PENDING_OPS_APPROVAL', val.trim());
                                  setConfirmState(prev => ({ ...prev, isOpen: false }));
                                }
                              });
                            }}
