...
export const ImportAssetModal = ({
  ...
  // Generate and download Excel Template matching the "Create Asset" form layout
  const handleDownloadTemplate = () => {
    const defaultSite = sites[0]?.name || "Skyrise 4B (SK4)";
    const defaultCategory = categories[0]?.name || "Cables";
    const defaultSupplier = suppliers[0]?.name || "No Supplier Assigned";

    const templateData = [
      {
        "Asset ID / Serial": "", // Blank for auto-generate
        "Location Site *": defaultSite,
        "Brand Name *": "Dell",
        "Model / Specification": "OptiPlex 7090 / i7 / 16GB",
        "Category *": defaultCategory,
        "Quantity *": 10,
        "Unit Cost (₱) *": 450.00,
        "Assigned Supplier": defaultSupplier,
        "Description": "Standard desktop workstation batch",
      },
      {
        "Asset ID / Serial": "AST-CUSTOM-001", // User provided ID example
        "Location Site *": defaultSite,
        "Brand Name *": "HP",
        "Model / Specification": "LaserJet Pro M404dn",
        "Category *": defaultCategory,
        "Quantity *": 1,
        "Unit Cost (₱) *": 12500.00,
        "Assigned Supplier": "",
    ...
    const colWidths = [
      { wch: 22 }, // Asset ID / Serial
      { wch: 25 }, // Location Site
      { wch: 20 }, // Brand Name
      { wch: 30 }, // Model / Specification
      { wch: 20 }, // Category
      { wch: 12 }, // Quantity
      { wch: 16 }, // Unit Cost
      { wch: 25 }, // Assigned Supplier
      { wch: 35 }, // Description
    ];
    ...
    reader.onload = (event) => {
      try {
        ...
        rawData.forEach((row: any, idx: number) => {
          ...
          const assetId = (findValue(["Asset ID / Serial", "Asset ID", "Serial"]) || "").toString().trim();
          const locationName = (findValue(["Location Site *", "Location Site", "Location", "Site"]) || "").toString().trim();
          const brandName = (findValue(["Brand Name *", "Brand Name", "Brand"]) || "").toString().trim();
          const modelSpec = (findValue(["Model / Specification", "Model", "Specification"]) || "").toString().trim();
          const categoryName = (findValue(["Category *", "Category"]) || "").toString().trim();
          const quantityRaw = findValue(["Quantity *", "Quantity", "Qty"]) ?? 1;
          const unitCostRaw = findValue(["Unit Cost (₱) *", "Unit Cost", "Unit Cost (P)", "Cost", "Price", "Unit Price"]) ?? 0;
          const supplierName = (findValue(["Assigned Supplier", "Supplier"]) || "").toString().trim();
          const description = (findValue(["Description"]) || "").toString().trim();

          const quantity = parseInt(quantityRaw, 10);
          const unitPrice = parseFloat(unitCostRaw);

          // Validation
          if (!brandName) {
            errList.push(`Row ${rowNum}: Brand Name is required.`);
          }
          if (!categoryName) {
            errList.push(`Row ${rowNum}: Category is required.`);
          }
          if (isNaN(quantity) || quantity <= 0) {
            errList.push(`Row ${rowNum}: Quantity must be a positive integer.`);
          }
          if (isNaN(unitPrice) || unitPrice < 0) {
            errList.push(`Row ${rowNum}: Unit Cost must be a valid number >= 0.`);
          }
          ...
          let matchedCategory = categories.find(c =>
            ...
            c.prefix?.toLowerCase() === categoryName.toLowerCase() ||
            (categoryName.toLowerCase().includes("cable") && c.name.toLowerCase().includes("cable")) ||
            (categoryName.toLowerCase().includes("headset") && c.name.toLowerCase().includes("headset"))
          );
          ...
          // Match supplier
          let matchedSupplier = suppliers.find(sup => sup.name.toLowerCase() === supplierName.toLowerCase());
          ...
          parsedRows.push({
            ...
            brandName,
            modelSpec,
            description: modelSpec ? (description ? `${description} | Model: ${modelSpec}` : `Model: ${modelSpec}`) : description,
            siteId: matchedSite?.id || sites[0]?.id || "",
            siteName: matchedSite?.name || "Default Site",
            categoryId: matchedCategory?.id || categories[0]?.id || "",
            categoryName: matchedCategory?.name || "Default Category",
            supplierId: matchedSupplier?.id || null,
            supplierName: matchedSupplier?.name || "None",
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      setErrors([`Error saving imported assets: ${err.message || "Failed"}`]);
    } finally {
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.15rem" }}>
            <h3 style={{ fontSize: "1.05rem", fontWeight: 700, color: "#0f172a", margin: 0, display: "flex", alignItems: "center", gap: "8px" }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#6366F1" strokeWidth="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" y1="3" x2="12" y2="15" /></svg>
              Import Assets from Excel
            </h3>
            <p style={{ fontSize: "0.78rem", color: "#64748b", margin: 0 }}>
              Batch upload inventory items using standard Excel template matching your Create Asset structure.
            </p>
          </div>
          <button
            type="button"
        ...
        <div style={{ padding: "1.5rem", overflowY: "auto", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
              <div style={{ width: 36, height: 36, borderRadius: "8px", backgroundColor: "#4F46E5", color: "#FFF", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              <div>
                ...
                <p style={{ fontSize: "0.75rem", color: "#4338CA", margin: 0 }}>Download pre-formatted Excel template matching the Create Asset fields.</p>
              </div>
            </div>
            <button
              type="button"
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              borderRadius: "8px",
              border: "none",
              background: previewData.length > 0 && errors.length === 0 ? "linear-gradient(135deg, #6366F1 0%, #4F46E5 100%)" : "#94A3B8",
              color: "#FFFFFF",
              fontSize: "0.82rem",
              fontWeight: 600,
          ...
          >
            {isProcessing ? "Importing Assets..." : `Import ${previewData.length > 0 ? previewData.length : ""} Asset(s)`}
          </button>
        </div>
